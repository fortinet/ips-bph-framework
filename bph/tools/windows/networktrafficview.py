...
class BphNetworkTrafficView(BphToolTemplateExecutor):
    def __init__(self, target_file=None, tool_name='networktrafficview', arch='x86', version='2.30'):
        super().__init__()
        self.load_tool_config_file(tool_name, arch, version, target_file=target_file)

    def start(self):
        self.logger.log(colored('START', 'yellow'))
        self.configuration.reporting.report_files = False
        self.configuration.execution.background_run = True
        self.actions.action = "start"
       
    def stop(self):
        self.logger.log(colored('STOP', 'yellow'))
