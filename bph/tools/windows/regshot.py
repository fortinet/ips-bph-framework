...
class BphRegShot(BphToolTemplateExecutor):

    def __init__(self, target_file=None, tool_name='regshot', arch='x86', version='1.9.0'):
        super().__init__()
        self.load_tool_config_file(tool_name, arch, version, target_file=target_file)

    def first_shot(self):
        self.logger.log(colored('FIRST SHOT', 'yellow'))
        self.configuration.reporting.report_files = False
        self.actions.action = "first_shot"
       
    def second_shot(self):
        self.logger.log(colored('SECOND SHOT', 'yellow'))
        self.configuration.reporting.report_files = False
        self.actions.action = "second_shot"

    def compare(self):
        self.logger.log(colored('SHOT DIFFING', 'yellow'))
